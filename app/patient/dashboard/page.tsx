...
export default function PatientDashboard() {
  ...
  const fetchClaims = async () => {
    try {
      const response = await fetch(`/api/claims?email=${user?.email}`)
      if (response.ok) {
  ...
  const totalApproved = approvedClaims.reduce((sum, claim) => sum + (claim.approvedAmount || 0), 0)

  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">Welcome back</h2>
        <p className="text-muted-foreground">Here's an overview of your claims and their status</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Your most recently submitted claims</CardDescription>
          </CardHeader>
          ...
          <CardFooter>
            <Button variant="outline" className="w-full" onClick={() => router.push("/patient/claims")}>
              View All Claims
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Average Approval Rate</span>
          ...
          <CardFooter>
            <Button className="w-full" onClick={() => router.push("/patient/submit-claim")}>
              Submit New Claim
              <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </CardFooter>
